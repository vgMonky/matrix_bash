...
END {
    if (direction == "up") {
        for (y = y_dim - 1; y >= 0; y--) {
            for (x = 0; x < x_dim; x++) {
                if (y == y_dim - 1) new_matrix[y][x] = 0;
                else if (matrix[y+1][x] == 1) new_matrix[y][x] = 1;
        ...
        for (x = x_dim - 1; x >= 0; x--) {
            for (y = 0; y < y_dim; y++) {
                if (x == x_dim - 1) new_matrix[y][x] = 0;
                else if (matrix[y][x+1] == 1) new_matrix[y][x] = 1;
                else if (matrix[y][x] == 1) new_matrix[y][x] = 0;
                else new_matrix[y][x] = 0;
            }
        }
